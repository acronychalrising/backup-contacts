## project: backup contacts

goal: write a script that pulls all contacts from contacts app and displays it as a list in a new note titled **backup contacts 9**, in my *backup contacts* folder.

display in the format: **1. full name: phone number**

bonus goal: compare the names with existing lists backup contacts 1-8.

## steps:
1. get full list of contacts from mac's contact app (done)
2. export it into aforementioned format (done)
3. add list to notes app (done)
4. bonus: normalize backup contacts 1-8 and check for duplicates